...
/**
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (userAndDepart == null) {
            ...
            if (url != null) {
                // 获取路径中的shopId
                String[] items = url.value().split("/");
                String pathId = null;
                for(int i = 0; i < items.length; i++) {
                    if ((items[i].equals("shops") || items[i].equals("departs")) && i + 1 < items.length) {
                        pathId = items[i + 1];
                        break;
                    }
                }
...
//            }
            ...
            if (!redisTemplate.hasKey(urlKey)) {
                ...
                Long privId = ((Integer) redisTemplate.opsForValue().get(urlKey)).longValue();
                logger.info("//////////////////////////////");
                logger.info(url.toString());
                logger.info("user: " + privId);
                logger.info(" url: " + redisTemplate.opsForSet().members(key));
                logger.info("//////////////////////////////");
