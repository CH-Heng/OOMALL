...
public class RoleDao {
    ...
    public ReturnObject loadRole(Long id) {
        try {
            ...
            // 角色被禁止,则退出
            if (rolePo != null && rolePo.getState() != null && rolePo.getState().equals(Role.State.FORBID.getCode())) {
     ...
     */
    public ReturnObject selectRole(Long id) {
        try {
            RolePo po = roleMapper.selectByPrimaryKey(id);
            if (po == null) {
                return new ReturnObject(ReturnNo.RESOURCE_ID_NOTEXIST);
            }
            return Common.getRetVo(new ReturnObject(po), Role.class);
        } catch (Exception e) {
            return new ReturnObject(ReturnNo.INTERNAL_SERVER_ERR, e.getMessage());
        }
    }
    ...
    public ReturnObject deleteRole(Long roleId, Long did) {
        try {
            ...
            if (ret == 0) {
                ...
                ReturnObject ret1=privDao.deleteRolePrivByRoleId(roleId);
     ...
     * @author 22920192204289 王文凯
     */
    public ReturnObject updateRoleState(Role bo) {
        try {
            RolePo po = roleMapper.selectByPrimaryKey(bo.getId());
            if (po == null) {
                return new ReturnObject(ReturnNo.RESOURCE_ID_NOTEXIST);
            }
            if (po.getState().equals(bo.getState())) {
                return new ReturnObject(ReturnNo.STATENOTALLOW);
            }

            Collection<String> relations = roleImpact(bo.getId());
            for (String s : relations) {
                redisUtil.del(s);
            }

            return updateRole(bo);
        } catch (Exception e) {
            return new ReturnObject(ReturnNo.INTERNAL_SERVER_ERR, e.getMessage());
        }
    }
    ...
    public ReturnObject updateRole(Role role) {
        RolePo po = Common.cloneVo(role, RolePo.class);
        ...
        try {
            if (roleMapper.selectByPrimaryKey(po.getId()) == null) {
                return new ReturnObject(ReturnNo.RESOURCE_ID_NOTEXIST);
            }

            int ret = roleMapper.updateByExampleSelective(po, example);
            if (ret == 0) {
                return new ReturnObject(ReturnNo.RESOURCE_ID_NOTEXIST);
            }
            return new ReturnObject();
...
//    }
     ...
     * @return
     */
    public ReturnObject<Boolean> isBaseRole(Long roleid) {
        try {
            RolePo rolePo = roleMapper.selectByPrimaryKey(roleid);
            if (rolePo == null) {
                return new ReturnObject<>(ReturnNo.RESOURCE_ID_NOTEXIST,false);
            }
            if (rolePo.getBaserole() != BASEROLE) {
                return new ReturnObject<>(ReturnNo.RESOURCE_ID_OUTSCOPE,false);
            }
            return new ReturnObject<>(ReturnNo.OK,true);
        } catch (Exception e) {
        ...
        try {
            ...
            for (int i = (page - 1) * pageSize; i <= lastIndex; i++) {
                ...
                if (obj instanceof String) {
                    ...
                    RolePo rolePo = roleMapper.selectByPrimaryKey(Long.parseLong(((String) obj).substring(begin)));
