...
public class UserDao {
    ...
    public ReturnObject setUsersProxy(UserProxy bo) {
        try {
            Collection<String> keys=userImpact(bo.getUserId());
            ...
            Collection<String> redisKeyToDeleted = userImpact(bo.getUserId());
    ...
    public ReturnObject removeUserProxy(Long id, Long userId) {
        try {
            Collection<String> keys=userImpact(userId);
            for (String key : keys) {
                if (redisUtil.hasKey(key)) {
                    redisUtil.del(key);
                }
            ...
            Collection<String> redisKeyToDeleted = userImpact(userProxyPo.getUserId());
            for (String key : redisKeyToDeleted) {
                redisUtil.del(key);
            }
            if (userProxyPo.getProxyUserId().equals(userId)){
                int ret = userProxyPoMapper.deleteByPrimaryKey(id);
                if (ret == 1) {
    ...
    public ReturnObject removeAllProxies(Long id, Long departId) {
        try {
            ...
            Collection<String> keys = userImpact(userProxyPo.getUserId());
            for (String key : keys) {
                if (redisUtil.hasKey(key)) {
                    redisUtil.del(key);
                }
    ...
    public ReturnObject getUserByAccount(String account) {
        ...
        if (null == users || users.isEmpty()) {
            ...
            if (userPo.getState() == User.State.DELETE.getCode().byteValue()) {
                return new ReturnObject<>(ReturnNo.AUTH_INVALID_ACCOUNT);
            }
    ...
    public ReturnObject loadUserPriv(Long userId, String jwt) {
        try {
            ...
            //最终用户   key: fu_id values: set{br_id};
            String aKey = String.format(FINALUSERKEY, userId);
            //被代理的用户     key: up_id  values: set{priv_id}
            //TODO:可能有错
            ...
            //遍历baserolekey  功能用户的redis key:br_id values:set{privId};
            for (Serializable brKey : brKeys) {
                ...
                if (!redisUtil.hasKey(brKeyStr)) {
                    int begin = brKeyStr.indexOf('_')+1;
                    Long roleId = Long.parseLong(brKeyStr.substring(begin));
...
//    }
    ...
    public ReturnObject<Object> modifyUserByVo(Long did, Long id, ModifyUserVo userVo, Long loginUser, String
            loginName) {
        UserPo user = userMapper.selectByPrimaryKey(id);
        if(user==null){
            return new ReturnObject<>(ReturnNo.RESOURCE_ID_NOTEXIST);
        }
    ...
    public ReturnObject<Object> changeUserState(Long did, Long id, Long loginUser, String loginName, User.State
            state) {
        UserPo user = userMapper.selectByPrimaryKey(id);
        if(user==null||user.getState()==User.State.DELETE.getCode().byteValue()){
            return new ReturnObject<>(ReturnNo.RESOURCE_ID_NOTEXIST);
        }
        if (!checkUserDid(id, did) && did != Long.valueOf(0)) {
            return new ReturnObject<>(ReturnNo.RESOURCE_ID_OUTSCOPE);
        }
        if(user.getState().byteValue()==state.getCode()){
    ...
    public ReturnObject<Object> resetPassword(ResetPwdVo vo) {
        ...
        try {
            ...
            userPoExample1.or(criteria_email);
...
//        }
    ...
    public ReturnObject<Object> modifyPassword(ModifyPwdVo modifyPwdVo) {
        ...
        Long id = ((Integer) redisUtil.get(key)).longValue();

        ReturnObject<Object> retObj = getUserPoById(id);
        if (retObj.getCode() != ReturnNo.OK) {
            if (retObj.getCode()==ReturnNo.RESOURCE_ID_NOTEXIST){
                return new ReturnObject<>(ReturnNo.AUTH_ID_NOTEXIST);
            }
            return retObj;
        }
        ...
        //name空 或  name与邮箱、手机号、用户名都不匹配
        if (modifyPwdVo.getName()==null||
                !(modifyPwdVo.getName().equals(userPo.getUserName())||modifyPwdVo.getName().equals(userPo.getEmail())||modifyPwdVo.getName().equals(userPo.getMobile()))){
            return new ReturnObject<>(ReturnNo.AUTH_ID_NOTEXIST);
        }
    ...
    public ReturnObject addUser(NewUserPo po, Long loginUser, String loginName) {
        ...
        userPo.setState(User.State.NORM.getCode().byteValue());
        userPo = (UserPo) baseCoder.code_sign(userPo, UserPo.class, userCodeFields, userSignFields, "signature");
        try {
    ...
    public ReturnObject getUserState() {
        ...
        for (User.State states : User.State.values()) {
...
/*    public ReturnObject updateUserAvatar(UserBo userBo) {
    ...
    public ReturnObject<VoObject> revokeRole(Long userid, Long roleid, Long did) {
        try {
            RolePo rolePo = roleDao.getRolePoByRoleId(roleid);
            UserPo userPo = userMapper.selectByPrimaryKey(userid);
            if(userPo==null||rolePo==null){
                return new ReturnObject<>(ReturnNo.RESOURCE_ID_NOTEXIST);
            }
            if ((checkUserDid(userid, did) && roleDao.checkRoleDid(roleid, did)) || did.equals(0L)) {
                //更新redis缓存
    ...
    public ReturnObject assignRole(UserRole userRole, Long did) {
        try {
            ...
            UserPo userPo = userMapper.selectByPrimaryKey(userRole.getUserId());
            if(userPo==null||rolePo==null){
                return new ReturnObject<>(ReturnNo.RESOURCE_ID_NOTEXIST);
            }
    ...
    public ReturnObject selectBaseRoles(Long userId, Long did, Integer page, Integer pageSize) {
        try {
            ...
            for (int i = (page - 1) * pageSize; i <= lastIndex; i++) {
                ...
                int begin = ((String)obj).indexOf('_')+1;
                RolePo rolePo = roleDao.getRolePoByRoleId(Long.parseLong(((String) obj).substring(begin)));
            ...
            info.setPages(baseroleKeyIds.size() / pageSize + 1);
            info.setTotal(baseroleKeyIds.size());
            return Common.getPageRetVo(new ReturnObject<>(info), RoleRetVo.class);
    ...
    public Collection<String> userImpact(Long userId) {
        ...
        for (UserProxyPo userProxyPo : userProxyPos) {
            if (uIds.add(userProxyPo.getUserId())) {
                ...
                String key1=String.format(USERPROXYKEY, userId);
                //TODO:可能有问题
//                if (redisUtil.hasKey(key1)){
//                    redisUtil.del(key1);
//                }
                keys.add(key1);
                String key2=String.format(FINALUSERKEY, userId);
//                if (redisUtil.hasKey(key1)){
//                    redisUtil.del(key1);
//                }
                keys.add(key2);
...
//    public ReturnObject selectTotalUsers(){
//        try{
//            UserPoExample example=new UserPoExample();
//            List<UserPo> userPos=userMapper.selectByExample(example);
//            return new ReturnObject(userPos);
...
//
//    }
    public ReturnObject checkRegister(NewUserPo newUserPo) {
        try {
            UserPoExample example = new UserPoExample();
            UserPoExample.Criteria criteria = example.createCriteria();
            criteria.andUserNameEqualTo(newUserPo.getUserName());
            UserPoExample.Criteria criteria1 = example.createCriteria();
            criteria1.andMobileEqualTo(newUserPo.getMobile());
            UserPoExample.Criteria criteria2 = example.createCriteria();
            criteria2.andEmailEqualTo(newUserPo.getEmail());
            example.or(criteria);
            example.or(criteria1);
            example.or(criteria2);
            List<UserPo> userPos=userMapper.selectByExample(example);
            ...
            if(userPo.getEmail().equals(newUserPo.getEmail())){
                return new ReturnObject(ReturnNo.EMAIL_REGISTERED);
            }
            if(userPo.getMobile().equals(newUserPo.getMobile())){
                return new ReturnObject(ReturnNo.MOBILE_REGISTERED);
            }
            if(userPo.getUserName().equals(newUserPo.getUserName())){
                return new ReturnObject(ReturnNo.USER_NAME_REGISTERED);
            }
            return new ReturnObject(ReturnNo.OK);
        } catch (Exception e) {
