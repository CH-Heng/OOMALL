...
public class UserService {
    ...
    public ReturnObject login(String account, String password, String ipAddr) {
        ...
        //对user对象进行检验
        if (user == null || !password.equals(user.getPassword())||user.getState()==User.State.DELETE.getCode().byteValue()) {
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public ReturnObject Logout(Long userId,String token) {
    ...
    public ReturnObject showUserInformation(Long id, Long did) {
        ReturnObject retObj = privilegeDao.getPriv(2L);
        if (retObj.getData() == null) {
            return retObj;
        }
        Privilege bo = (Privilege) retObj.getData();
        if (bo.getState().equals(Privilege.State.FORBIDDEN.getCode())) {
        ...
        if(userRetVo.getSign()==1){
            return new ReturnObject(ReturnNo.RESOURCE_FALSIFY,userRetVo);
        }
     ...
     */
    /*@Transactional(rollbackFor = Exception.class)
    public ReturnObject modifyUserInformation(Long id, ModifyUserVo modifyUserVo, Long userId, String userName) {
        User user = (User) Common.cloneVo(modifyUserVo, User.class);
        user.setId(id);
    ...
    public ReturnObject<PageInfo<Object>> showUsers(Long did, String userName, String mobile, String email, Integer page, Integer pageSize) {
        ...
        if (wrongSign&&(userName!=null||mobile!=null||email!=null)) {
            return Common.getPageRetVo(new ReturnObject<>(ReturnNo.RESOURCE_FALSIFY,proxyRetVoPageInfo), UserRetVo.class);
        }
        else{
            return Common.getPageRetVo(new ReturnObject<>(proxyRetVoPageInfo), UserRetVo.class);
        }
    }
