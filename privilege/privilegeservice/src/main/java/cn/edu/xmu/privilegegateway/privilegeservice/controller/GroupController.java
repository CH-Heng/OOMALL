...
public class GroupController {
    ...
    @Audit(departName = "departs")
    @GetMapping("/departs/{did}/groups/{id}/baseroles")
    public Object getgroupbaseroles(@PathVariable Long did,@PathVariable Long id, @RequestParam(required = false) Integer page,
                                    @RequestParam(required = false) Integer pageSize,
                                    @LoginUser @ApiIgnore @RequestParam(required = false) Long userId,
                                    @RequestParam(required = false) @LoginName @ApiIgnore String userName)
    {
        if(did==null || did<0)
            return new ReturnObject(ReturnNo.FIELD_NOTVALID,"部门名不合法");
        ReturnObject returnObject =  groupService.getgroupsbaseroles(did,id,page,pageSize);
        return Common.decorateReturnObject(returnObject);

    }
    @ApiOperation(value = "获得组的角色")
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "header", dataType = "String", name = "authorization", value = "Token", required = true),
            @ApiImplicitParam(name="did", value="部门id", required = true, dataType="int", paramType="path"),
            @ApiImplicitParam(name="id", value="组id", required = true, dataType="int", paramType="path")
    ...
    @Audit(departName = "departs")
    @GetMapping("/departs/{did}/groups/{id}/roles")
    public Object getgrouproles(@PathVariable Long did,@PathVariable Long id, @RequestParam(required = false) Integer page,
                                @RequestParam(required = false) Integer pageSize,
                                @LoginUser @ApiIgnore @RequestParam(required = false) Long userId,
                                @RequestParam(required = false) @LoginName @ApiIgnore String userName)
    {
        ...
        ReturnObject returnObject =  groupService.getgroupsroles(did,id,page,pageSize);
        return Common.decorateReturnObject(returnObject);
    ...
    public Object insertgroup(@LoginUser Long userId,
                              ...
                              @PathVariable(name="did") Long did,
    ...
    public Object getparGroup(@PathVariable Long did,@PathVariable Long id, @RequestParam(required = false) Integer page,
                              ...
                              @LoginUser @ApiIgnore @RequestParam(required = false) Long userId,
                              @RequestParam(required = false) @LoginName @ApiIgnore String userName){
        ReturnObject<PageInfo<Object>> returnObject =  groupService.getparGroup(id,page,pageSize);
        return Common.decorateReturnObject(Common.getPageRetVo(returnObject,Group.class) );
    ...
    @PutMapping(value = "/departs/{did}/groups/{id}/forbid")
    @Audit(departName = "departs")
