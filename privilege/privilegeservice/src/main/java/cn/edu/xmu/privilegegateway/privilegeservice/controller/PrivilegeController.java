...
public class PrivilegeController {
    ...
    @Audit(departName = "departs")
    @GetMapping("/departs/{did}/roles/{id}")
    public Object selectRole(@LoginUser Long userId,
                             @Depart Long departId,
                             @PathVariable("did") Long did,
                             @PathVariable("id") Long id) {
        logger.debug("delete role");

        ReturnObject retObj = roleService.selectRole(id);
    ...
    public Object insertRole(@PathVariable("did") Long did,
        ...
        Role role = Common.cloneVo(vo, Role.class);
        role.setDepartId(did);
        role.setBaserole((byte) 0);
        role.setState(Role.State.NORM.getCode());
    ...
    public Object forbidUser(@Depart Long did,@PathVariable Long id,@LoginUser Long loginUser,@LoginName String loginName) {
    ...
    @Audit
    @GetMapping("logout")
    public Object logout(@LoginUser Long userId,@RequestHeader String authorization){
        if(userId==null){
            return Common.decorateReturnObject(new ReturnObject(ReturnNo.AUTH_INVALID_JWT));
        }
        logger.debug("logout: userId = "+userId);
        return Common.decorateReturnObject(userService.Logout(userId,authorization));
    }
    ...
    public Object getProxies(@PathVariable("did") Long departId,
        ...
        ReturnObject<List> returnObject = userProxyService.getProxies(userId, proxyUserId, departId,begin,end, page, pageSize);
        return Common.decorateReturnObject(returnObject);
    }
     ...
     * @param pageSize
     * @return
     * createdBy 郎秀晨 2021/12/22
     */
    @Audit
    @GetMapping("/self/proxies")
    public Object getProxiesSelf(@LoginUser Long userId,
                                 @RequestParam(value = "page", required = false) Integer page,
                                 @RequestParam(value = "pageSize", required = false) Integer pageSize){
...
//====================以上
    ...
    public Object register(@Validated @RequestBody NewUserVo newUserVo, BindingResult bindingResult){
        ...
        if(newUserVo.getDepartId()==null){
            newUserVo.setDepartId(-1L);
        }
    ...
    public Object modifyUserInformation(@LoginUser Long userId,
                                        ...
                                        BindingResult bindingResult
            , HttpServletResponse httpServletResponse){
        Object res = Common.processFieldErrors(bindingResult, httpServletResponse);
        if (res != null) {
            return res;
        }
        ReturnObject ret =userService.modifyUserInfo(0L,userId,modifyUserVo,userId,userName);
        //ReturnObject ret =userService.modifyUserInformation(userId,modifyUserVo,userId,userName);
        return Common.decorateReturnObject(ret);
    }
//    /**
//     * 注册用户上传头像
...
//    @Audit(departName = "departs") // 需要认证
    ...
    public Object addToDepart(@PathVariable Long id,@PathVariable Long did) {
        Long loginUser = 1L;
        String loginName = "13088admin";
