...
public class GroupDao {
    ...
    private final static int BANED = 1;
...
//                    //修改
//                    int begin = uKey.indexOf('_')+1;
//                    String id = uKey.substring(begin);
...
//                }
    ...
    */
    ...
    public ReturnObject<RetGroup> insertGroup(Group group, Long loginUserId, String loginUserName) {
        ...
        {
            return new ReturnObject(ReturnNo.GROUP_RELATION_EXIST,"重复定义用户组");
        }
    ...
    public ReturnObject<RetGroup> updateGroup(Long id, Group group,Long loginUserId, String loginUserName) {
        try {
            Collection<String> keys=groupImpact(id);
            ...
            int ret = groupPoMapper.updateByPrimaryKeySelective(groupPo);
            GroupPo groupPo1 = groupPoMapper.selectByPrimaryKey(id);
            RetGroup temp=(RetGroup) Common.cloneVo(groupPo1,RetGroup.class);
            for (String key : keys) {
            ...
            if (ret == 0) {
                return new ReturnObject(ReturnNo.RESOURCE_ID_NOTEXIST,"更新失败");
            } else {
    ...
    {
        Collection<String> ids=groupImpact(groupRelation.getGroupSId());
        ...
        try{
            GroupPo groupPo1=groupPoMapper.selectByPrimaryKey(groupRelation.getGroupPId());
            GroupPo groupPo2=groupPoMapper.selectByPrimaryKey(groupRelation.getGroupSId());
            if(groupPo1==null||groupPo2==null)
            {
                return new ReturnObject<>(ReturnNo.RESOURCE_ID_NOTEXIST);
            }
            ...
            if (ret == 0) {
                ...
                temp.setId(groupRelationPo.getGroupSId());
                temp.setName(groupPo2.getName());
                temp.setSign((byte) 0);
    ...
    public ReturnObject deleteGroupRelation(Long id, Long userId, String userName) {
        ...
        GroupPo groupPo=groupPoMapper.selectByPrimaryKey(groupRelationPo.getGroupSId());
        ...
        try {
            ...
            RetGroup temp=(RetGroup) Common.cloneVo(groupPo,RetGroup.class);
            temp.setSign((byte) 0);
            for(String key:ids)
            {
                if(redisUtil.hasKey(key)){
     ...
     * createdBy:  Weining Shi
     */
    public ReturnObject insertUserGroup(UserGroup userGroup, Long userId, String userName) {
        Collection<String> ids;
        ...
        try{
            ...
            ReturnObject returnObject = userDao.getUserPoById(userGroupPo.getUserId());
            if(returnObject.getCode()!=ReturnNo.OK){
                return returnObject;
            }
            retObj = new ReturnObject(Common.cloneVo(returnObject.getData(), UserVo.class));
            for(String key:ids)
            {
                if(redisUtil.hasKey(key)){
                    redisUtil.del(key);
    ...
    public ReturnObject<PageInfo<UserRelation>> getusersBygid(Long did, Long id, Integer page, Integer pageSize) {
        ...
        List<UserGroupPo> po;
        List<UserGroupBo> userGroupBos;
        List<UserRelation> userRelations=new ArrayList<>();
        try {
            PageHelper.startPage(page,pageSize);
            po = userGroupPoMapper.selectByExample(example);////////////////////////////////////////////
            logger.debug("getGroupIdByUserId: userId = " + id + "groupNum = " + po.size());
            userGroupBos=Common.listDecode(po,UserGroupBo.class,baseCoder,groupUserCodeFields,groupUserSignFields,"signature",true);
            int check=0;
            for(UserGroupBo bo:userGroupBos) {
                ReturnObject ret=userDao.getUserById(bo.getUserId());
                ...
                if (bo.getSignature() == null)
                {
                    ...
                    user.setSign((byte)0);
                UserRelation userRelationVo=(UserRelation) Common.cloneVo(user,UserRelation.class);
                userRelations.add(userRelationVo);
            }
            ReturnObject ret=new ReturnObject(new PageInfo<>(userRelations));
            if(check==1)
                return Common.getPageRetVo(new ReturnObject(ReturnNo.RESOURCE_FALSIFY,"签名错误",new PageInfo<>(userRelations)),UserRelation.class);
            else{
