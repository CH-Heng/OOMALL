...
public class BaseRoleAndPrivilegeTest {
    ...
    public void testGetAllPrivilegesStates() throws Exception{
        ...
        String expected="{\"errno\":0,\"data\":[{\"code\":0,\"name\":\"NORMAL\"},{\"code\":1,\"name\":\"FORBIDDEN\"}],\"errmsg\":\"成功\"}";
    ...
    public void testAddBaseRolePrivilegesWithErrorRole() throws Exception{
        String responseString = this.mvc.perform(post("/departs/0/baseroles/1/privileges/190")
                .header("authorization", token)
                .contentType("application/json;charset=UTF-8"))
                .andExpect(status().isForbidden())
                .andExpect(content().contentType("application/json;charset=UTF-8"))
    ...
    {
        String responseString = this.mvc.perform(delete("/departs/0/baseroles/1/privileges/111")
                .header("authorization", token)
                .contentType("application/json;charset=UTF-8"))
    ...
    public void testSelectBaseRolePrivs() throws  Exception
    {
        String responseString = this.mvc.perform(get("/departs/0/baseroles/88/privileges")
                ...
                .andExpect(content().contentType("application/json;charset=UTF-8"))
                .andReturn().getResponse().getContentAsString();
        String exstr=" {\"errno\":0,\"errmsg\":\"成功\"}";
    ...
    {
        String responseString = this.mvc.perform(get("/departs/0/baseroles/-1/privileges")
                .header("authorization", token)
                .contentType("application/json;charset=UTF-8")
                .param("page","1")
                .param("pageSize","10"))
    ...
    public void getPrivilege() throws  Exception
    {
        String responseString = this.mvc.perform(get("/departs/0/privileges")
                ...
                .andExpect(content().contentType("application/json;charset=UTF-8"))
                .andReturn().getResponse().getContentAsString();
        String expected="{\"errno\":0,\"data\":{\"total\":1,\"pages\":1,\"pageSize\":10,\"page\":1,\"list\":[{\"id\":2,\"name\":\"查看任意用户信息\",\"url\":\"/privilege/departs/{id}/users/{id}\",\"requestType\":0,\"creator\":{\"id\":1,\"name\":\"admin\"},\"modifier\":{\"id\":null,\"name\":null},\"sign\":0}]},\"errmsg\":\"成功\"},\"errmsg\":\"成功\"}";
        JSONAssert.assertEquals(expected, responseString,false);
    }
