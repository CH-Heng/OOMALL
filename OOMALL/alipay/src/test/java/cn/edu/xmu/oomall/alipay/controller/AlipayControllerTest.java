...
import cn.edu.xmu.oomall.alipay.service.mq.CallBackListener;
import cn.edu.xmu.oomall.alipay.service.mq.NotifyService;
...
class AlipayControllerTest {
    ...
    void refundQuery() throws Exception{
        ...
        String responseString2 = this.mockMvc.perform(MockMvcRequestBuilders.post("/internal/alipay/gateway.do")
                ...
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString(StandardCharsets.UTF_8);
        String expectedResponse2="{\"sign\":\"ERITJKEIJKJHKKKKKKKHJEREEEEEEEEEEE\",\"alipay_trade_fastpay_refund_query_response\":{\"code\":\"10000\",\"msg\":\"Success\",\"out_trade_no\":\"2\",\"out_request_no\":\"1\",\"total_amount\":100,\"refund_amount\":90,\"refund_status\":\"REFUND_SUCCESS\",\"trade_no\":\"2013112011001004330000121536\"}}";
    ...
    public void paymentRocketMQTest()throws Exception{
        ...
        String json= JSON.toJSONString(notifyBody);
...
//        rocketMQTemplate.sendOneWay("payment-success-topic", message);
        callBackListener.onMessage(json);
        notifyService.onMessage(notifyBody);
    }
}