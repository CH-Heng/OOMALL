...
public class RegionControllerTest {
    ...
    @Test
    public void getRegionInfoTest1()throws Exception{
        String responseString = this.mvc.perform(get("/internal/region/1").header("authorization", adminToken).contentType("application/json;charset=UTF-8"))
                ...
                .andExpect(content().contentType("application/json;charset=UTF-8"))
                .andReturn().getResponse().getContentAsString();
        String expectedResponse = "{\"errno\":0,\"data\":{\"id\":1,\"name\":\"中国\"},\"errmsg\":\"成功\"}";
        JSONAssert.assertEquals(expectedResponse, responseString, true);
    }

    @Test
    public void getRegionInfoTest2()throws Exception{
        String responseString = this.mvc.perform(get("/internal/region/0").header("authorization", adminToken).contentType("application/json;charset=UTF-8"))
                .andExpect(status().isNotFound())
                .andExpect(content().contentType("application/json;charset=UTF-8"))
                .andReturn().getResponse().getContentAsString();
        String expectedResponse = "{\"errno\":504,\"errmsg\":\"操作的资源id不存在\"}";
