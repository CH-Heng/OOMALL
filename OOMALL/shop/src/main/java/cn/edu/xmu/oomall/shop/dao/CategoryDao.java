...
public class CategoryDao {
    ...
    public ReturnObject insertCategory(CategoryPo categoryPo) {
        try {
            ...
            //更新redis
            String subKey = redisKeyForFindParentCategory + categoryPo.getId();
            redisUtil.set(subKey, categoryPo.getId(), categoryTimeout);
            CategoryPoExample example = new CategoryPoExample();
            CategoryPoExample.Criteria criteria = example.createCriteria();
            List<CategoryPo> categoryPos;
            criteria.andPidEqualTo(categoryPo.getPid());
            // 去数据库查询
            categoryPos = categoryPoMapper.selectByExample(example);
            List<Category> categories = new ArrayList<>();
            CategoryRetVo category = cloneVo(categoryPo, CategoryRetVo.class);
            for (CategoryPo categoryPo1 : categoryPos) {
                Category c = cloneVo(categoryPo1, Category.class);
                categories.add(c);
            }
            CategoryList categoryList;
            categoryList = new CategoryList();
            categoryList.setCategoryList(categories);
            String key = redisKeyForSubList + categoryPo.getPid();
        ...
        try {
            ...
            if (null != categoryList) {
                for (Category category:categoryList.getCategoryList()){
                    if (category.getId()==314L){
                        if (ok){
                            category.setCommissionRatio(1);
                        }
                        ok=true;
                    }
